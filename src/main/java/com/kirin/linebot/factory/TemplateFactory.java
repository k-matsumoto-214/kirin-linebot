...
public class TemplateFactory {
    ...
    private static final String DESCRIPTION_TYPE = "予約したい人を選んでください";
    ...
    public TemplateMessage reservationTypeMessage(List<ReservationType> reservationTypes) {
        ...
        List<Action> datetimePickerActions = reservationTypes.stream()
                .map(reservationType -> DatetimePickerAction.OfLocalDate
                        ...
                        .max(MAX_DATE)
                        .data("{" +
                                "\"reservationName\":\"" + reservationType.getReservationName().getValue() + "\"," +
                                "\"reservationTime\":\"" + reservationType.getReservationTime().getValue() + "\"" +
                                "}")
                        .build())
